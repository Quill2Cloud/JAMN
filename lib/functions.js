...
logError = function(type, error, reason, showUser, message, action, actionRoute) {
  if (showUser) {
    Session.set('errorType', type);
    Session.set('errorMessage', message);
    Session.set('errorAction', action);
    Session.set('errorRoute', actionRoute);
  }
  var properties = {
    showUser: showUser,
    message: message,
  ...
  Meteor.call('logEvent', getEventData(type, reason, properties));
};
